...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # ODOEN: 2. Implement this TEST function.
...
def run_test_draw_points_on_circle():
    ...
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  69 purple dots.'
    window = rg.RoseWindow(600, 600, title)
    circle = rg.Circle(rg.Point(200, 200), 200)
    draw_points_on_circle(window, circle, 69, 'purple')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  49 slices, thin (thickness=3) purple lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    pizza(window, circle, 49, 'purple', 3)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # Tests 1 and 2 (on the same window):
    title = ('POLYGON test 3:  14 segments with thin pruple lines;'
             + ' 6 with medium red lines.')
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(300, 300), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 14, 'purple', 3)
    window.continue_on_mouse_click()
    window.close_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  45 purple lines on yell0w circle,'
             + ' hops = 13.')
    window = rg.RoseWindow(1000, 1000, title)

    circle = rg.Circle(rg.Point(300, 365), 250)
    circle.fill_color = 'yellow'
    fancy_polygon(window, circle, 45, 13, 'purple', 3)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(number_of_lines):
        point1 = points[counter]
        counter = counter + hops_to_next_point
        if counter >= len(points):
            counter = counter%number_of_lines
        point2 = points[counter]
